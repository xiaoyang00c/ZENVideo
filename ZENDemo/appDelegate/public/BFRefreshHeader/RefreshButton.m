...
{
    self.image = [UIImage imageNamed:imageNaem];
}
...
{
    if (!loadingView) {
        loadingView = [[UIImageView alloc] initWithFrame:CGRectMake(-6, -6, self.frame.size.width+12, self.frame.size.height+12)];
        loadingView.image = [UIImage imageNamed:@"bf_refresh_quan"];
    ...
    animation.toValue = [NSNumber numberWithFloat:6 * M_PI];

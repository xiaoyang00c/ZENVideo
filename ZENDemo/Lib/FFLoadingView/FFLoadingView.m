...
- (void)setTipsText:(NSString *)tipsText {
    if (!_tipsLabel) {
        _tipsLabel = [[UILabel alloc] initForAutoLayout];
        _tipsLabel.textColor = self.strokeColor;
        _tipsLabel.font = [UIFont systemFontOfSize:15.f];
        [self addSubview:_tipsLabel];
        [_tipsLabel autoCenterInSuperview];
    }
...
//    gradientLayer1.colors = @[(__bridge id)[UIColor colorWithHexString:@"#45b2cf" alpha:0.8].CGColor, (__bridge id)[UIColor colorWithHexString:@"#45b2cf" alpha:0.1].CGColor];
...
- (void)drawCircleAnimation:(NSNumber*)from end:(NSNumber*)end duration:(CGFloat)duration {
    ...
    [roundPath addArcWithCenter:CGPointMake(rectSelf.size.width / 2, rectSelf.size.height / 2 ) radius:CGRectGetHeight(rectSelf) / 2 - self.lineWidth/2 startAngle:- 0.5 * M_PI endAngle:1.5 * M_PI clockwise:YES];
